...
def tds_voltage_to_ppm(voltage, temperature):
...
def get_stable_reading(channel, conversion_func=None, temp_func=None):
    print(f"\nStep {channel + 2}: Place {SENSOR_INFO[channel]['name']} sensor in water.")
    input("Press Enter when the sensor is in the water...")
    print(f"\nGetting stable reading for {SENSOR_INFO[channel]['name']}...")
    ...
    while (time.time() - start_time) < 10:
        ...
        ADC.ADS1263_SetChannel(8)  # Force ADC to read GND before switching
        time.sleep(1)

        ADC.ADS1263_SetChannel(channel)
        ADC.ADS1263_WaitDRDY()

        while (time.time() - sample_start) < 2:  # Collect data for 2 seconds
            voltage = ADC.ADS1263_Read_ADC_Data() * REF / 0x7FFFFFFF
        ...
        print(f"Reading: {avg_sampled_reading:.2f} {SENSOR_INFO[channel]['unit']}")

        if len(readings) > 5:  # Use last 5 readings for stability
            avg = sum(readings[-5:]) / 5
            threshold = STABILITY_THRESHOLDS[SENSOR_INFO[channel]['name']]
            if all(abs(r - avg) / avg < threshold for r in readings[-5:]):
                stable = True
                break

    avg_reading = sum(readings) / len(readings)
    print(f"\nStable {SENSOR_INFO[channel]['name']} Reading: {avg_reading:.2f} {SENSOR_INFO[channel]['unit']}")
    return avg_reading
...
We use **special pH solutions (4, 7, and 10)** to help it **learn!** 🧪
    ...
    for ph in ph_values:
        ...
        while (time.time() - start_time) < 10:
            ...
            ADC.ADS1263_SetChannel(8)  # Set ADC to GND to clear floating voltage
            time.sleep(1)  # Allow time to stabilize

            # ✅ **Now switch to the pH sensor channel (channel 2)**
            ADC.ADS1263_SetChannel(2)
            ADC.ADS1263_WaitDRDY()
